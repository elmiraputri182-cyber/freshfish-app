...
class EditOperasionalPage extends StatefulWidget {
  final Map data;

  const EditOperasionalPage({super.key, required this.data});

  @override
  State<EditOperasionalPage> createState() => _EditOperasionalPageState();
}

class _EditOperasionalPageState extends State<EditOperasionalPage> {
  final namaController = TextEditingController();
  final noHpController = TextEditingController();
  final alamatController = TextEditingController();
  final lokasiController = TextEditingController();
  final tangkapanController = TextEditingController();
  ...
  DateTime estimasiKembali = DateTime.now();

  @override
  void initState() {
    super.initState();

    namaController.text = widget.data["nama_nelayan"] ?? "";
    noHpController.text = widget.data["no_hp"] ?? "";
    alamatController.text = widget.data["alamat"] ?? "";
    lokasiController.text = widget.data["lokasi"] ?? "";
    tangkapanController.text = widget.data["jenis_tangkapan"] ?? "";
    keteranganController.text = widget.data["keterangan"] ?? "";
    status = widget.data["status"] ?? "Melaut";

    try {
      if (widget.data["tanggal_berangkat"] != null) {
        tanggalBerangkat = DateTime.parse(widget.data["tanggal_berangkat"]);
      }
    } catch (_) {}

    try {
      if (widget.data["estimasi_kembali"] != null) {
        estimasiKembali = DateTime.parse(widget.data["estimasi_kembali"]);
      }
    } catch (_) {}
  }
  ...
  Future<void> updateOperasional() async {
    ...
    try {
      final response = await http.post(
        Uri.parse("${Api.baseUrl}/operasional/edit_operasional.php"),
        body: {
          "id_nelayan": widget.data["id_nelayan"].toString(),
          ...
          "jenis_tangkapan": tangkapanController.text,
      ...
      if (json["success"] == true) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              json["message"]?.toString() ?? "Gagal menyimpan perubahan",
            ),
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: loading ? null : updateOperasional,
                      child: loading
                          ? const SizedBox(
                              height: 20,
                              width: 20,
